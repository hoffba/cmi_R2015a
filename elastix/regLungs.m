...
function stat = regLungs(cmiObj0,cmiObj1,varargin)
...
%               'FixedPyramid'  = [1x(3*R)] for R resolutions (3 values for each)
%               'MovingPyramid' = [1x(3*R)] for R resolutions (3 values for each)
...
if (nargin>=2) && isa(cmiObj0,'CMIclass') && isa(cmiObj1,'CMIclass') ...
    ...
    if (length(varargin)==1) && ischar(varargin{1})
        % Define default parameters for mouse and human lungs
        if strcmp(varargin{1},'mouse')
            varargin = {'SP_a',[82,42,20,10,5],...
                        'SP_A',50,...
                        'Iterations',[5000,5000,1000,1000,1000],...
                        'FixedPyramid',[16,16,16,8,8,8,4,4,4,2,2,2,1,1,1],...
                        'MovingPyramid',[16,16,16,8,8,8,4,4,4,2,2,2,1,1,1],...
                        'Dilate',[5,5,5],...
                        'Clamp',[-2000,0]};
    ...
    addParamValue(p,'T0',[],@(x)isvector(x)&&(length(x)==6));
...
%     clinchk = any(cmiObj0.img.dims(1:3).*cmiObj0.img.voxsz > 50);
