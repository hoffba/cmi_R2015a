...
function setTx0(self,x,fvoxsz,fdims,fdir,forigin,varargin)
...
elseif (nargin>3)
    ...
    addRequired(p,'Direction',@(x)isvector(x)&&(length(x)==6));
    addRequired(p,'Position',@(x)isvector(x)&&(length(x)==3));
    addParameter(p,'DefaultPixelValue',0,@isnumeric);
    addParameter(p,'FixedImageLandmarks',[],@isnumeric);
    parse(p,x,fvoxsz,fdims,fdir,forigin,varargin{:});
    ...
    t.Origin = p.Results.Position;
    t.CenterOfRotationPoint = zeros(1,3);
    t.Direction = [p.Results.Direction,cross(p.Results.Direction(1:3),p.Results.Direction(4:6))];
