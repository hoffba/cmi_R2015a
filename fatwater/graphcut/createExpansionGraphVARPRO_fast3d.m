...
%
% Input arguments:
%   - residual: the 3D residual array (of size NUM_FMS x SX x SY x SZ)
...
function [A,Aunsc] = createExpansionGraphVARPRO_fast3d( residual, dfm, lambda, size_clique, cur_ind, step )

% Allocate space for the adjacency matrix A (which completely describes the graph)
[L,sx,sy,sz] = size(residual);
s = sx*sy*sz;
num_nodes = s + 2;
...
for dx = -size_clique:size_clique
    for dy = -size_clique:size_clique
        for dz = -size_clique:size_clique
            dist = sqrt(dx^2+dy^2+dz^2);
            if dist>0
                validmapi = X+dx>=1 & X+dx<=sx & Y+dy>=1 & Y+dy<=sy & Z+dz>=1 & Z+dz<=sz;
                validmapj = X-dx>=1 & X-dx<=sx & Y-dy>=1 & Y-dy<=sy & Z-dz>=1 & Z-dz<=sz;
