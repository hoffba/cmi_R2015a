...
function fm = graphCutIterations3d(imDataParams,algoParams,residual,lmap,cur_ind )
...
fm = zeros(sx,sy,sz);
fprintf('Starting graph cut iterations ...\n');
for kg=1:algoParams.NUM_ITERS
    ...
    if rand < prob_bigJump % If we're making a "jumpmin" move.
        cur_ind2(1,:,:,:) = cur_ind;
        repCurInd = repmat(cur_ind2,[numLocalMin,1,1,1]);
        if cur_sign>0
        ...
        cur_step = all_jump*ones(sx,sy,sz);
    ...
    [A] = createExpansionGraphVARPRO_fast3d( residual, dfm, lambdamap, algoParams.size_clique,cur_ind, cur_step);
    %   else
    %     [A] = createExpansionGraphVARPRO_fast3d( residual, dfm, lambdamap, algoParams.size_clique,cur_ind, cur_step);
    %   end
    A(A<0)=0;
    ...
    cut1b(end) = 0;
    if DEBUG == 1
        fprintf('compareGC = %f , %f',sum(sum(A(cut1b==1, cut1b==0))),sum(sum(A(cut1==1, cut1==0))));
    ...
    if sum(sum(A(cut1b==1, cut1b==0))) <= sum(sum(A(cut1==1, cut1==0)))
        ...
        cut = reshape(cut1(2:end-1)==0,sx,sy,sz);
    ...
    if DISPLAY_ITER == 1
        ...
        figure(hf),
        subplot(1,3,1),imagesc(fm(:,:,round(sz/2)),[-600 600]);axis off equal tight,colormap gray;colorbar;
        subplot(1,3,2),imagesc(squeeze(fm(:,round(sy/2),:)),[-600 600]);axis off equal tight,colormap gray;colorbar;
        subplot(1,3,3),imagesc(squeeze(fm(round(sx/2),:,:)),[-600 600]);axis off equal tight,colormap gray;colorbar;
...
%         end
    ...
    fprintf('   Iteration %u/%u complete.\n',kg,algoParams.NUM_ITERS);
end
