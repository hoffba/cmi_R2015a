...
function d2 = getQuadraticApprox3d( residual, dfm )

[NUM_FMS,sx,sy,sz] = size(residual);
resoffset = (0:(sx*sy*sz-1))'*NUM_FMS;

[~,iminres] = min(residual(10:end-9,:,:,:),[],1);
iminres = squeeze(iminres + 9);

d2 = (residual((iminres(:)+1) + resoffset) + residual((iminres(:)-1) + resoffset) - 2*residual((iminres(:)) + resoffset)  )/dfm^2;

d2 = reshape(d2,[sx sy sz]);
