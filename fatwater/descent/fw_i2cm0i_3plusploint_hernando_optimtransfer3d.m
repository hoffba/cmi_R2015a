...
function outParams = fw_i2cm0i_3plusploint_hernando_optimtransfer3d( imDataParams, algoParams )
...
Dx = spalloc(np,np,2*np);
Dy = spalloc(np,np,2*np);
Dxy1 = spalloc(np,np,2*np);
Dxy2 = spalloc(np,np,2*np);
    ...
    for kx=1:sx
        for ky=1:sy
            for ka=1:num_acqs
                ...
                curGamma(kx,ky,:,:,ka) = curPhi*(curPhi'*curPhi)\curPhi';
...
for kit = 1:NUM_ITERS
    ...
    [fmstep,~] = pcg(curHessTotal,-curGradTotal,1e-6,400);
    ...
    if DEBUG_MODE == 1
        ...
        fprintf('curroughness = %f',curroughness);
