function [img,label,fov,dcmdata] = readDICOMtest(varargin)
...
for ifn = 1:nf
    ...
    if isempty(j)
        ...
        dcmdata(j).SeriesDescription = val;
        val = '';
    ...
    if isfield(tinfo,'ImagePositionPatient')
        val = tinfo.ImagePositionPatient(3);
    elseif isfield(tinfo,'SliceLocation')
        dcmdata(j).SlcLoc(k) = tinfo.SliceLocation;
    end
    dcmdata(j).SlcLoc(k) = val;
    
    % Read slice data from file:
    dcmdata(j).img(:,:,k) = dicomread(tinfo);
...
if length(dcmdata)>1
    str = strcat('(',cellfun(@(x)num2str(length(x)),{dcmdata(:).SlcLoc},...
                             'UniformOutput',false),...
                 ' Slices)',{dcmdata(:).SeriesDescription});
...
img = double(dcmdata.img);
dcmdata.img = [];
[d(1),d(2),d(3)] = size(img);
fov = [dcmdata.PixelSpacing(:)',1] .* d;
if length(dcmdata.SlcLoc)>1
    fov(3) = abs(diff(dcmdata.SlcLoc([1,end]))) * (1+1/(d(3)-1));
elseif ~isempty(dcmdata.SlcThk)
    fov(3) = d(3) * dcmdata.SlcThk;
end
label = {dcmdata.SeriesDescription};
dcmdata(:)
