...
for ifn = 1:nf
    ...
    if (ifn==1) && isfield(tinfo,'Manufacturer') % Check manufacturer to read private tags:
        ddir = fileparts(which('cmi'));
        if strncmpi(tinfo.Manufacturer,'philips',7)
            dicomdict('set',fullfile(ddir,'read','dicom-dict-Philips.txt'));
        elseif strncmpi(tinfo.Manufacturer,'ge',2)
            dicomdict('set',fullfile(ddir,'read','dicom-dict-GE.txt'));
        elseif strncmpi(tinfo.Manufacturer,'siemens',7)
...
%     end
    ...
    for i = 1:length(dcmdata)
        [dcmdata(i).SlcLoc,ix] = sort(dcmdata(i).SlicePos);
    ...
    n4d = d(4);
    if (n4d==1) && all(isfield(dcmdata,{'TR','TE'}))
...
%                                     'UniformOutput',false);
    ...
    fov = [dcmdata.PixelSpacing(:)',1] .* d([2,1,3]);
