...
function [im,dim,par] = Get_mrd_3D1(filename,reordering1, reordering2)
...
% reorder the data explicitly - slow code, rewrite if needed
for a=1:no_expts
    for b=1:no_echoes
        for c=1:no_slices
            for d=1:no_views
                for e=1:no_views_2
                    m_C_1(a,b,c,ord2(e),ord(d),:) = m_C(1+n:no_samples+n); % sequential ordering                    
                    n=n+no_samples;
                end
            end
        end
...
if numel(ppr_text)>0
    ...
    PPR_type_2 = [4 7 9:11 15:21 25 31 33 41 49:52];
    ...
    PPR_type_6 = 39; % VAR_ARRAY keyword
