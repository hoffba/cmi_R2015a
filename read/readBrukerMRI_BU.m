...
switch tname
    ...
    case '2dseq' % Recontructed image
        ...
        nd = p.RecoDim;
        d = p.RECO_size;
        d(4) = p.RecoObjectsPerRepetition;
        d(5) = p.RecoNumRepetitions;
        if nd==2
        ...
        img = permute(reshape(img,d),[2,1,3,4,5]);
        fov = fov([2,1,3]);
        ...
        img = permute(reshape(img,d(2),d(1),[]),[2,1,3,4]);
        fov = fov([2,1,3]);
        m = p.RECO_map_slope;
        b = p.RECO_map_offset;
        ...
        if p.RecoDim==2
            for islc = 1:d(3)
                img(:,:,islc) = img(:,:,islc) / m(islc) - b(islc);
            end
        elseif p.RecoDim==3
            for i4d = 1:d(4)
