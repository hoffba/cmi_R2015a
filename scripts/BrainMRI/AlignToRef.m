function [Y,Yaff] = AlignToRef(X,Xaff,XaffRef)
...
for i = 1 : 3
    for j = 1 : 3
        ...
        COST(i,j) = - abs(a'*b)/norm(a)/norm(b);
        
    end
end
[assignment,cost] = munkres(COST);
[idx,~] = find(assignment);

% Permute dimensions if needed
v2r = Xaff;
v2r(1:3,1:3)=Xaff(1:3,idx);
V=permute(V,idx');

% Flip dimensions if needed
for d = 1 : 3
    if sum(v2r(:,d) .* XaffRef(:,d)) < 0
        v2r(1:3,d) = - v2r(1:3,d);
        v2r(1:3,4) = v2r(1:3,4) - v2r(1:3,d)*(size(V,d)-1);
        V = flipdim(V,d);
    end
end
