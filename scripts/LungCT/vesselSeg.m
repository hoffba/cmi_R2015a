...
%vesselSeg.m Creates vessel segmentations and CSA maps for subject
%specified in file_ids
% Inputs:
...
function testsample = importfile(filename, dataLines)
%IMPORTFILE1 Import data from a text file
%  TESTSAMPLE = IMPORTFILE1(FILENAME) reads data from text file FILENAME
%  for the default selection.  Returns the data as a table.
%
%  TESTSAMPLE = IMPORTFILE1(FILE, DATALINES) reads data for the
%  specified row interval(s) of text file FILENAME. Specify DATALINES as
%  a positive scalar integer or a N-by-2 array of positive scalar
%  integers for dis-contiguous row intervals.
%
%  Example:
%  testsample = importfile1("Z:\CT_Lung\Matlab_Scripts\Emily\BeaumontBatch\VesselSeg_code\test_sample.csv", [2, Inf]);
%
%  See also READTABLE.
%
% Auto-generated by MATLAB on 09-Apr-2021 12:54:58
...
% Import the data
testsample = readtable(filename, opts);
...
function[files] = getFiles(subj_dir)
    all_files_obj = dir(subj_dir);
    files = [];
    for i = 1:length(all_files_obj)
        if (all_files_obj(i).bytes > 0)
            files = [string(files); string(all_files_obj(i).name)];
        end
    end
end
