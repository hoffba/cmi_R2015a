...
function [atMap, laaMap, percentatMap, percentlaaMap, meanvalatMap, meanvallaaMap] = ScatNet(imgFlNm, msFlNm, flag1)
...
var = 2; % to blur the image if it has a lot of noise
...
res = FctSeg(gaussianBlur(roi,var),ws,segn,1,omega,beta);
...
% niftiwrite(im2uint8(atMap),[imgFlNm(1:end-4),'_scatternet'],'Compressed',true);
% close all;
