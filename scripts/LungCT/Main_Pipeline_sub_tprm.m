function res = Main_Pipeline_sub_tprm(basename,expfname,procdir)
...
elseif ~isempty(ins_reg)
    writeLog(fn_log,'Calculating PRM...\n');
    [prm10,~] = pipeline_PRM(img.mat,img.info,logical(img.label),ins_reg,...
...
%% Save Results Table:
writetable(struct2table(res,'AsArray',true),fullfile(procdir,[res.ID,'_tPRM-ONLY_Results.csv']));
