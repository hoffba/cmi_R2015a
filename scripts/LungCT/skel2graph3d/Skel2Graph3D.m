...
link = struct('n1',{},'n2',{},'point',{},'label',{});
for i=1:cc2.NumObjects
    node(i).idx = cc2.PixelIdxList{i};
    node(i).links = [];
    node(i).conn = [];
    [x,y,z]=ind2sub(d,node(i).idx);
    node(i).comx = mean(x);
    node(i).comy = mean(y);
    node(i).comz = mean(z);
    node(i).ep = 0;
...
for i=1:length(node)
    [x,y,z] = ind2sub(d,node(i).idx);
    node(i).idx = sub2ind(d-2,x-1,y-1,z-1);
...
for i=1:length(link)
    [x,y,z] = ind2sub(d,link(i).point);
    link(i).point = sub2ind(d-2,x-1,y-1,z-1);
end
