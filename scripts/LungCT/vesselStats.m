...
np = nnz(mask);
voxvol = 6.25^3 ;
T.VOLUME = np * voxvol; % Volume in microliters, assume voxel size of (0.625mm)^3
T.VESSEL_VOLUME = nnz(V) * voxvol;
...
if any(C,'all')
    ...
    T.VESSEL_VOXELS_5DOWN = N * voxvol;
    T.VESSEL_VOXELS_5UP = (nnz(V) - N) * voxvol;
