function [T,ver] = pipeline_vesselseg_noresample(ct,seg,info,save_path,opts_in,fn_log)
...
%               orient = orientation tensor of the image
    ...
    %% Parse inputs
    if (ischar(ct) || isstring(ct)) && (ischar(seg) || isstring(seg)) && (ischar(info) || isstring(info))
