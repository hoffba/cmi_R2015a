function res = CTlung_Pipeline_sub_sextants(procdir,opts)
...
try
    ...
    fn_res = fullfile(procdir,[ID,'_PipelineResults_sextants.csv']);
    % tPRM labels
    ...
    regionnames = {'WholeLung','RL','LL','RUS','RMS','RLS','LUS','LMS','LLS'};
    ...
    if img(1).flag
        if exist(opts.fn.exp_seg,'file')
            img(1).label =  yacta2sext(cmi_load(1,[],opts.fn.exp_seg));
        else
            img(1).flag = false;
        end
    end
    if img(2).flag
        if exist(opts.fn.ins_seg,'file')
            img(2).label = yacta2sext(cmi_load(1,[],opts.fn.ins_seg));
        else
            img(2).flag = false;
        end
    end
