...
if ndims(image) ~= 3
    error('The input image must be a three-dimensional array.');
end
if sum(mod(window,2))~=3
...
mean = imboxfilt3(image,window,'padding',padding);
% Local contrast
local_contrast = minmaxfilt3(image,'max',window) - minmaxfilt3(image,'min',window);
