% CMI script
function runBatchPRMImbio(cmiObj)
...
for i = 1:length(fnames)
    ...
    %stat = cmiObj.loadImg(0,fnames{i}(1:2));
    
    % load 2 images and also mask
    stat = cmiObj.loadImg(0,fnames{i}(1:3));
    if ~stat || cmiObj.img.dims(4)<2
        ...
        % Separate out lung from Imbio DICOM mask file:
        % lung 1, lung 2, airway 3
        % Note: threshold expects min in row 1 and max in 2
        cmiObj.img.threshold(3,[0.5;2.9]); % change from 1;2 to 1;3 5 may 2014
        cmiObj.img.thresh2mask(3);
...
disp('Done with script batchPRMImbio!')
