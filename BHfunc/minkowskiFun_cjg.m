...
%         normval = p.wnum(i)*voxvol*ones(1,nmf); % For normalizing V, S, B [ORIGINAL CODE]
        normval = repmat(feval(p.func,wmask,p.voxsz,0), [1,4]); % Added by CJG 20210323
%         apply process to denominator as numerator
...
p = nan(1,length(ord));
if ismember(0,ord)
    p(ord==0) = imVolumeEstimate_noEdgeCor(BW,voxsz);
%     'no edge'
end
if ismember(1,ord)
