...
function [atMap, laaMap, percentatMap, percentlaaMap, meanvalatMap, meanvallaaMap] = ScatterNetAT_PEDS(imgFlNm, msFlNm, flag1)
...
var = 0.85; % to blur the image if it has a lot of noise
laaval = -856;
...
percentlaaMap = length(find(laaMap))/length(find(mask2));
beta = percentlaaMap;
