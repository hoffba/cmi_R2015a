...
area_tri = zeros(nf,1);
for i = 1:nf
    p = TR.Points(TR.ConnectivityList(i,:),:);
    area_tri(i,1) = 0.5*norm(cross(p(2,:)-p(1,:),p(3,:)-p(1,:)));
end
...
for i = 1:np
    ...
    if ~ismember(i,bndry_edge)
        neib_tri = vertexAttachments(TR,i);
        ...
        for j = 1:length(neib_tri{1})
            ...
            for k=1:3
                if TR.ConnectivityList(neib,k)==i
            ...
            switch k
                case 1
                    mc_vec = mc_vec + (v1(neib,:)/tan(ang_tri(neib,3)) - ...
                                       v3(neib,:)/tan(ang_tri(neib,2)));
                case 2
                    mc_vec = mc_vec + (v2(neib,:)/tan(ang_tri(neib,1)) - ...
                                       v1(neib,:)/tan(ang_tri(neib,3)));
                case 3
                    mc_vec = mc_vec + (v3(neib,:)/tan(ang_tri(neib,2)) - ...
                                       v2(neib,:)/tan(ang_tri(neib,1)));
            end
            ...
            if ang_tri(neib,k) >= pi/2
                ...
                if any(ang_tri(neib,:) >= pi/2)
                    ...
                    for m=1:3
                        if m~=k
                            ll = mod(m,3)+1; %% p1==>l2   ,p2==>l3   ,p3==>l1
                            csum = csum + l_edg(neib,ll)^2/tan(ang_tri(neib,m));
                        end
                    end
                    a_mixed(i) = a_mixed(i) + csum/8;
                end
            end
            
            %%%% normal vector at each vertex
            %%%% weighted average of normal vecotors of neighbour triangles
            wi = 1/norm([ f_center(neib,1)-TR.Points(i,1) ,...
                          f_center(neib,2)-TR.Points(i,2) ,...
                          f_center(neib,3)-TR.Points(i,3) ]);
